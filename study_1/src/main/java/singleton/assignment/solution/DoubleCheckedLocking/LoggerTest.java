...
public class LoggerTest {
    public static void main(String[] args) {
        ...
        logger1.initialize("application_dcl.log", LogLevel.INFO);
        ...
        System.out.println("\nDouble-Checked Locking 싱글톤 로거 테스트 완료!");

        // 인스턴스 재설정 테스트 (일반적으로 사용하지 않음)
        System.out.println("\n인스턴스 재설정 테스트 (일반적인 사용 패턴은 아님):");
        Logger.reset();
        Logger logger3 = Logger.getInstance();
        System.out.println("재설정 후 새 인스턴스 생성됨: " + (logger1 != logger3));
    }
}
