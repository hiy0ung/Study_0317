...
public class ProductInventoryService {
    ...
    public BigDecimal calculateDiscountedPrice(String productId, double discountRate) {
        return findProduct(productId)
                ...
                .orElse(BigDecimal.ZERO); // null 이면 0을 반환하게 함
    }

    // 상품 정보 요약을 문자열로 반환하는 메서드
    // 기본메세지는 "상품 정보 없음" (String.format 사용...?)
    // 상품이름 (재고: 00개, 가격: 00원) >> 형태로 출력
    public String infoProduct(String productId) {
        return findProduct(productId)
    ...
    public String getProductSummary(String productId) {
        return findProduct(productId)
                .map(product -> String.format("%s (재고: %d개, 가격: %s원)", // BigDecimal 한 번 더 변환해야해서 그냥 %s로 둠?
                        product.getName(),
                        product.getStock(),
                        product.getPrice()))
                .orElse("상품 정보 없음");
